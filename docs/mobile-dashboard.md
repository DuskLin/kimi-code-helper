...
- 访问码、Tunnel 凭证、TLS 私钥使用 Electron safeStorage 加密；Linux basic_text 后端不允许保存。网页不在 URL 或 localStorage 保存凭证。
